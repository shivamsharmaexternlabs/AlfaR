...
const Signature = ({ signnatureFileFun, signnatureFileError, setSignnatureFileError }) => {


    const canvasRef = useRef(null);

    const [clearSignature, setClearSignature] = useState()
    const [signatureDownlaod, setSignatureDownlaod] = useState(false)

    const clear = () => setClearSignature(o => !o)
    useEffect(() => {
        if (canvasRef.current) {
            // Access canvas methods and properties
    ...
    const handleGetCanvas = useCallback(() => {
        ...
        signnatureFileFun(newFun(canvas, filename))
        setSignnatureFileError(newFun(canvas, filename))
    ...
    console.log("sjdgcsd", signnatureFileError)



    return (<>
        <div className='signature-main w-100'>
        ...
        <div>
            {signnatureFileError == false && <center> <p className='text-danger   mt-2 bt-2'> please signature here</p></center>}

            <div className='clear-signature  ' onClick={clear}>clear</div>
            <button type='button' className='saveBtn' onClick={handleGetCanvas}>save</button>
        </div>
